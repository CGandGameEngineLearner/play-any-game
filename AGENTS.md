...
- 🖼️ **游戏窗口截图** - 支持硬件加速渲染的游戏窗口（使用 node-screenshots）
- 🖱️ **游戏兼容点击** - 使用 SendInput API，对游戏兼容性更好
- 📝 **智能文件名** - 截图文件名自动包含游戏窗口标题
- ⏰ **24小时挂机** - 可以长时间运行，解放双手
- 📊 **任务汇报** - 完成后自动汇报刷取成果和统计
...
# 截图（指定窗口，文件名包含标题）
npx tsx scripts/index.ts screenshot "原神"

# 截取指定游戏窗口（推荐，支持硬件加速渲染）
npx tsx scripts/index.ts capture "原神"

# 点击坐标（使用 SendInput API，游戏兼容性好）
npx tsx scripts/index.ts click 540 820

# 查找按钮（基于颜色匹配）
npx tsx scripts/index.ts find "开始"
```
...
- 应保留关键操作的截图用于调试和分析
- 截图文件名格式：`screenshot_窗口标题_时间戳.png`
...
| 依赖包 | 用途 |
|--------|------|
| `node-screenshots` | 窗口截图（支持游戏硬件加速渲染） |
| `@nut-tree-fork/nut-js` | 桌面自动化框架（可选） |
| `minimist` | 命令行参数解析 |

### 技术实现细节

#### 截图功能
- 使用 `node-screenshots` 实现
- 支持硬件加速渲染的游戏窗口（原神、崩坏星穹铁道等）
- 截图文件名自动包含窗口标题

#### 点击功能
- 使用 Windows `SendInput` API
- 相比过时的 `Mouse_event` API，对游戏兼容性更好
- 支持左键点击、右键点击、鼠标移动
