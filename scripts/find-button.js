/**
 * find-button.js - 按钮查找功能
 * 
 * 在截图中查找按钮位置，基于颜色匹配和区域分析。
 * 使用 PowerShell + System.Drawing 实现（快速采样版本）。
 */
 ...
 * 根据按钮名称查找对应的颜色配置
 */
function getColorConfigForButton(buttonName) {
 ...
 * 使用网格采样代替逐像素扫描
 */
export async function findButton(imagePath, buttonName) {
    ...
    try {
        ...
        console.error('[find-button] 查找按钮失败: ' + err.message);
        return null;
    }
}
 ...
 * 获取所有匹配的按钮位置
 */
export async function findAllButtons(imagePath, buttonName) {
    let absolutePath = imagePath;
    if (!path.isAbsolute(imagePath)) {
        absolutePath = path.join(__dirname, '..', imagePath);
    }
 ...
 * 快速查找颜色区域（使用采样）
 */
async function findColorRegionsFast(imagePath, colorConfig) {
    const escapedPath = imagePath.replace(/'/g, "''");
    const step = 5; // 采样步长，每5像素采一次
    ...
    try {
        const { stdout, stderr } = await execAsync('powershell -ExecutionPolicy Bypass -File "' + scriptPath + '"', { maxBuffer: 1024 * 1024 * 10 });
